                ...
                <form method="POST" action="{{ route('admin.jadwal.update', $jadwal) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="kelas_id" name="kelas_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($kelas as $kelasItem)
                                <option value="{{ $kelasItem->id }}" {{ old('kelas_id', $jadwal->kelas_id) == $kelasItem->id ? 'selected' : '' }}>{{ $kelasItem->nama_kelas }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="mata_kuliah_id" name="mata_kuliah_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            @foreach ($mataKuliah as $mata)
                                <option value="{{ $mata->id }}" {{ old('mata_kuliah_id', $jadwal->mata_kuliah_id) == $mata->id ? 'selected' : '' }}>{{ $mata->nama }} ({{ $mata->kode }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="tahun_ajaran" name="tahun_ajaran" type="text" class="mt-1 block w-full" value="{{ old('tahun_ajaran', $jadwal->tahun_ajaran) }}" required />
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="ruangan" name="ruangan" type="text" class="mt-1 block w-full" value="{{ old('ruangan', $jadwal->ruangan) }}" />
                    ...
                    <div class="mb-4 mt-4">
                        ...
                        <select id="status" name="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            <option value="1" {{ old('status', $jadwal->status) ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ ! old('status', $jadwal->status) ? 'selected' : '' }}>Nonaktif</option>
                        </select>
