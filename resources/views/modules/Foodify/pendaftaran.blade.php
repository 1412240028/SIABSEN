...
@section('content')
    ...
    <table border="1" width="700" align="center" cellpadding="6" cellspacing="0">
        ...
        <tr align="center">
            <td>3</td>
            <td><b>Proses Order Lebih Cepat</b></td>
        ...
        <table border="1" width="700" align="center" cellpadding="6" cellspacing="0">
            ...
            <tr>
                ...
                <td>
                    <select name="tgl">
                        @for ($i = 1; $i <= 31; $i++)
                            <option value="{{ $i }}" {{ ($i == (int)($tgl_edit ?? 1)) ? 'selected' : '' }}>
                                {{ $i }}
                            </option>
                        @endfor
                    </select>
                    ...
                    <select name="bln">
                        @for ($i = 1; $i <= 12; $i++)
                            <option value="{{ $i }}" {{ ($i == (int)($bln_edit ?? 1)) ? 'selected' : '' }}>
                                {{ namaBulan($i) }}
                            </option>
                        @endfor
                    </select>
                    ...
                    <select name="thn">
                        @for ($y = 1990; $y <= 2030; $y++)
                            <option value="{{ $y }}" {{ ($y == (int)($thn_edit ?? 2000)) ? 'selected' : '' }}>
                                {{ $y }}
                            </option>
                        @endfor
                    </select>
